...
.HANDLE_BREAK:
    j USERRET_MACHINE
800002d8:	5980006f          	j	80000870 <USERRET_MACHINE>
...
800005e4:	807f02b7          	lui	t0,0x807f0
...
.LC0:
    addi t0, t0, -XLEN
80000604:	ffc28293          	addi	t0,t0,-4 # 807efffc <KERNEL_STACK_INIT+0xfffefffc>
    addi sp, sp, -XLEN
...
80000660:	00732023          	sw	t2,0(t1)
    ...
    j WELCOME                       // 进入主线程
80000664:	0040006f          	j	80000668 <WELCOME>

80000668 <WELCOME>:

WELCOME:
    la s1, monitor_version          // 装入启动信息
80000668:	00001497          	auipc	s1,0x1
8000066c:	b0448493          	addi	s1,s1,-1276 # 8000116c <monitor_version>
    lb a0, 0(s1)
80000670:	00048503          	lb	a0,0(s1)
.Loop0:
    addi s1, s1, 0x1
80000674:	00148493          	addi	s1,s1,1
    jal WRITE_SERIAL                // 调用串口写函数
80000678:	995ff0ef          	jal	ra,8000000c <WRITE_SERIAL>
    lb a0, 0(s1)
8000067c:	00048503          	lb	a0,0(s1)
    bne a0, zero, .Loop0            // 打印循环至0结束符
80000680:	fe051ae3          	bnez	a0,80000674 <WELCOME+0xc>

80000684:	0040006f          	j	80000688 <SHELL>
...
80000688:	a09ff0ef          	jal	ra,80000090 <READ_SERIAL>

    ori t0, zero, 'R'
8000068c:	05206293          	ori	t0,zero,82
    beq a0, t0, .OP_R
80000690:	06550863          	beq	a0,t0,80000700 <.OP_R>
    ori t0, zero, 'D'
80000694:	04406293          	ori	t0,zero,68
    beq a0, t0, .OP_D
80000698:	0a550263          	beq	a0,t0,8000073c <.OP_D>
    ori t0, zero, 'A'
8000069c:	04106293          	ori	t0,zero,65
    beq a0, t0, .OP_A
800006a0:	0c550e63          	beq	a0,t0,8000077c <.OP_A>
    ori t0, zero, 'G'
800006a4:	04706293          	ori	t0,zero,71
    beq a0, t0, .OP_G
800006a8:	10550c63          	beq	a0,t0,800007c0 <.OP_G>
    ori t0, zero, 'T'
800006ac:	05406293          	ori	t0,zero,84
    beq a0, t0, .OP_T
800006b0:	00550863          	beq	a0,t0,800006c0 <.OP_T>

    li a0, XLEN                     // 错误的操作符，输出 XLEN，用于区分 RV32 和 RV64
800006b4:	00400513          	li	a0,4
    jal WRITE_SERIAL                 // 把 XLEN 写给 term
800006b8:	955ff0ef          	jal	ra,8000000c <WRITE_SERIAL>
    j .DONE                         
800006bc:	2900006f          	j	8000094c <.DONE>

800006c0 <.OP_T>:

.OP_T:                              // 操作 - 打印页表
    addi sp, sp, -3*XLEN
800006c0:	ff410113          	addi	sp,sp,-12
    STORE s1, 0(sp)
800006c4:	00912023          	sw	s1,0(sp)
    STORE s2, XLEN(sp)
800006c8:	01212223          	sw	s2,4(sp)
...
#else
    li s1, -1
800006cc:	fff00493          	li	s1,-1
#endif
    STORE s1, 2*XLEN(sp)
800006d0:	00912423          	sw	s1,8(sp)
    addi s1, sp, 2*XLEN
800006d4:	00810493          	addi	s1,sp,8
    li s2, XLEN
800006d8:	00400913          	li	s2,4
.LC0:
    lb a0, 0(s1)           // 读取字节
800006dc:	00048503          	lb	a0,0(s1)
    addi s2, s2, -1                 // 滚动计数器
800006e0:	fff90913          	addi	s2,s2,-1
    jal WRITE_SERIAL                 // 写入串口
800006e4:	929ff0ef          	jal	ra,8000000c <WRITE_SERIAL>
    addi s1, s1, 0x1                // 移动打印指针
800006e8:	00148493          	addi	s1,s1,1
    bne s2, zero, .LC0              // 打印循环
800006ec:	fe0918e3          	bnez	s2,800006dc <.OP_T+0x1c>

    LOAD s1, 0x0(sp)
800006f0:	00012483          	lw	s1,0(sp)
    LOAD s2, XLEN(sp)
800006f4:	00412903          	lw	s2,4(sp)
    addi sp, sp, 3*XLEN
800006f8:	00c10113          	addi	sp,sp,12

    j .DONE
800006fc:	2500006f          	j	8000094c <.DONE>

80000700 <.OP_R>:

.OP_R:                              // 操作 - 打印用户空间寄存器
    addi sp, sp, -2*XLEN                 // 保存s1,s2
80000700:	ff810113          	addi	sp,sp,-8
    STORE s1, 0(sp)
80000704:	00912023          	sw	s1,0(sp)
    STORE s2, XLEN(sp)
80000708:	01212223          	sw	s2,4(sp)

    la s1, uregs
8000070c:	007f0497          	auipc	s1,0x7f0
80000710:	8f448493          	addi	s1,s1,-1804 # 807f0000 <_sbss>
    ori s2, zero, 31*XLEN               // 计数器，打印 31 个寄存器
80000714:	07c06913          	ori	s2,zero,124
.LC1:
    lb a0, 0(s1)           // 读取字节
80000718:	00048503          	lb	a0,0(s1)
    addi s2, s2, -1                 // 滚动计数器
8000071c:	fff90913          	addi	s2,s2,-1
    jal WRITE_SERIAL                 // 写入串口
80000720:	8edff0ef          	jal	ra,8000000c <WRITE_SERIAL>
    addi s1, s1, 0x1                // 移动打印指针
80000724:	00148493          	addi	s1,s1,1
    bne s2, zero, .LC1              // 打印循环
80000728:	fe0918e3          	bnez	s2,80000718 <.OP_R+0x18>

    LOAD s1, 0(sp)                    // 恢复s1,s2
8000072c:	00012483          	lw	s1,0(sp)
    LOAD s2, XLEN(sp)
80000730:	00412903          	lw	s2,4(sp)
    addi sp, sp, 2*XLEN
80000734:	00810113          	addi	sp,sp,8
    j .DONE
80000738:	2140006f          	j	8000094c <.DONE>

8000073c <.OP_D>:

.OP_D:                              // 操作 - 打印内存num字节
    addi sp, sp, -2*XLEN                 // 保存s1,s2
8000073c:	ff810113          	addi	sp,sp,-8
    STORE s1, 0(sp)
80000740:	00912023          	sw	s1,0(sp)
    STORE s2, XLEN(sp)
80000744:	01212223          	sw	s2,4(sp)

    jal READ_SERIAL_XLEN
80000748:	9e5ff0ef          	jal	ra,8000012c <READ_SERIAL_XLEN>
    or s1, a0, zero                 // 获得addr
8000074c:	000564b3          	or	s1,a0,zero
    jal READ_SERIAL_XLEN
80000750:	9ddff0ef          	jal	ra,8000012c <READ_SERIAL_XLEN>
    or s2, a0, zero                 // 获得num
80000754:	00056933          	or	s2,a0,zero

.LC2:
    lb a0, 0(s1)                    // 读取字节
80000758:	00048503          	lb	a0,0(s1)
    addi s2, s2, -1                 // 滚动计数器
8000075c:	fff90913          	addi	s2,s2,-1
    jal WRITE_SERIAL                 // 写入串口
80000760:	8adff0ef          	jal	ra,8000000c <WRITE_SERIAL>
    addi s1, s1, 0x1                // 移动打印指针
80000764:	00148493          	addi	s1,s1,1
    bne s2, zero, .LC2              // 打印循环
80000768:	fe0918e3          	bnez	s2,80000758 <.OP_D+0x1c>

    LOAD s1, 0(sp)                    // 恢复s1,s2
8000076c:	00012483          	lw	s1,0(sp)
    LOAD s2, XLEN(sp)
80000770:	00412903          	lw	s2,4(sp)
    addi sp, sp, 2*XLEN
80000774:	00810113          	addi	sp,sp,8
    j .DONE
80000778:	1d40006f          	j	8000094c <.DONE>

8000077c <.OP_A>:

.OP_A:                              // 操作 - 写入内存num字节，num为4的倍数
    addi sp, sp, -2*XLEN                 // 保存s1,s2
8000077c:	ff810113          	addi	sp,sp,-8
    STORE s1, 0(sp)
80000780:	00912023          	sw	s1,0(sp)
    STORE s2, 4(sp)
80000784:	01212223          	sw	s2,4(sp)

    jal READ_SERIAL_XLEN
80000788:	9a5ff0ef          	jal	ra,8000012c <READ_SERIAL_XLEN>
    or s1, a0, zero                 // 获得addr
8000078c:	000564b3          	or	s1,a0,zero
    jal READ_SERIAL_XLEN
80000790:	99dff0ef          	jal	ra,8000012c <READ_SERIAL_XLEN>
    or s2, a0, zero                 // 获得num
80000794:	00056933          	or	s2,a0,zero
    srl s2, s2, 2                   // num除4，获得字数
80000798:	00295913          	srli	s2,s2,0x2
.LC3:                               // 每次写入一字
    jal READ_SERIAL_WORD              // 从串口读入一字
8000079c:	911ff0ef          	jal	ra,800000ac <READ_SERIAL_WORD>
    sw a0, 0(s1)                    // 写内存一字
800007a0:	00a4a023          	sw	a0,0(s1)
    addi s2, s2, -1                 // 滚动计数器
800007a4:	fff90913          	addi	s2,s2,-1
    addi s1, s1, 4                  // 移动写指针
800007a8:	00448493          	addi	s1,s1,4
    bne s2, zero, .LC3              // 写循环
800007ac:	fe0918e3          	bnez	s2,8000079c <.OP_A+0x20>
    ...
    LOAD s1, 0(sp)                    // 恢复s1,s2
800007b0:	00012483          	lw	s1,0(sp)
    LOAD s2, XLEN(sp)
800007b4:	00412903          	lw	s2,4(sp)
    addi sp, sp, 2*XLEN
800007b8:	00810113          	addi	sp,sp,8
    j .DONE
800007bc:	1900006f          	j	8000094c <.DONE>

800007c0 <.OP_G>:

.OP_G:
    jal READ_SERIAL_XLEN            // 获取addr
800007c0:	96dff0ef          	jal	ra,8000012c <READ_SERIAL_XLEN>
    mv s10, a0                      // 保存到 s10
800007c4:	00050d13          	mv	s10,a0

    ori a0, zero, TIMERSET          // 写TIMERSET(0x06)信号
800007c8:	00606513          	ori	a0,zero,6
    jal WRITE_SERIAL                 // 告诉终端用户程序开始运行
800007cc:	841ff0ef          	jal	ra,8000000c <WRITE_SERIAL>

#ifdef ENABLE_INT
    csrw mepc, s10                // 用户程序入口写入EPC
800007d0:	341d1073          	csrw	mepc,s10
    li a0, MSTATUS_MPP_MASK
800007d4:	00002537          	lui	a0,0x2
800007d8:	80050513          	addi	a0,a0,-2048 # 1800 <INITLOCATE-0x7fffe800>
    csrc mstatus, a0     // 设置 MPP=0 ，对应 U-mode
800007dc:	30053073          	csrc	mstatus,a0
#endif

    la ra, uregs              // 定位用户空间寄存器备份地址
800007e0:	007f0097          	auipc	ra,0x7f0
800007e4:	82008093          	addi	ra,ra,-2016 # 807f0000 <_sbss>
    STORE sp, TF_ksp(ra)           // 保存栈指针
800007e8:	0820a023          	sw	sp,128(ra)

    // LOAD x1,  TF_ra(ra)
    LOAD sp, TF_sp(ra)
800007ec:	0040a103          	lw	sp,4(ra)
    LOAD gp, TF_gp(ra)
800007f0:	0080a183          	lw	gp,8(ra)
    LOAD tp, TF_tp(ra)
800007f4:	00c0a203          	lw	tp,12(ra)
    LOAD t0, TF_t0(ra)
800007f8:	0100a283          	lw	t0,16(ra)
    LOAD t1, TF_t1(ra)
800007fc:	0140a303          	lw	t1,20(ra)
    LOAD t2, TF_t2(ra)
80000800:	0180a383          	lw	t2,24(ra)
    LOAD s0, TF_s0(ra)
80000804:	01c0a403          	lw	s0,28(ra)
    LOAD s1, TF_s1(ra)
80000808:	0200a483          	lw	s1,32(ra)
    LOAD a0, TF_a0(ra)
8000080c:	0240a503          	lw	a0,36(ra)
    LOAD a1, TF_a1(ra)
80000810:	0280a583          	lw	a1,40(ra)
    LOAD a2, TF_a2(ra)
80000814:	02c0a603          	lw	a2,44(ra)
    LOAD a3, TF_a3(ra)
80000818:	0300a683          	lw	a3,48(ra)
    LOAD a4, TF_a4(ra)
8000081c:	0340a703          	lw	a4,52(ra)
    LOAD a5, TF_a5(ra)
80000820:	0380a783          	lw	a5,56(ra)
    LOAD a6, TF_a6(ra)
80000824:	03c0a803          	lw	a6,60(ra)
    LOAD a7, TF_a7(ra)
80000828:	0400a883          	lw	a7,64(ra)
    LOAD s2, TF_s2(ra)
8000082c:	0440a903          	lw	s2,68(ra)
    LOAD s3, TF_s3(ra)
80000830:	0480a983          	lw	s3,72(ra)
    LOAD s4, TF_s4(ra)
80000834:	04c0aa03          	lw	s4,76(ra)
    LOAD s5, TF_s5(ra)
80000838:	0500aa83          	lw	s5,80(ra)
    LOAD s6, TF_s6(ra)
8000083c:	0540ab03          	lw	s6,84(ra)
    LOAD s7, TF_s7(ra)
80000840:	0580ab83          	lw	s7,88(ra)
    LOAD s8, TF_s8(ra)
80000844:	05c0ac03          	lw	s8,92(ra)
    LOAD s9, TF_s9(ra)
80000848:	0600ac83          	lw	s9,96(ra)
    // LOAD s10, TF_s10(ra)
    LOAD s11, TF_s11(ra)
8000084c:	0680ad83          	lw	s11,104(ra)
    LOAD t3, TF_t3(ra)
80000850:	06c0ae03          	lw	t3,108(ra)
    LOAD t4, TF_t4(ra)
80000854:	0700ae83          	lw	t4,112(ra)
    LOAD t5, TF_t5(ra)
80000858:	0740af03          	lw	t5,116(ra)
    LOAD t6, TF_t6(ra)
8000085c:	0780af83          	lw	t6,120(ra)
...
80000864:	00c08093          	addi	ra,ra,12 # 8000086c <.USERRET_USER>
    mret                                // 进入用户程序
80000868:	30200073          	mret

8000086c <.USERRET_USER>:
    jr s10
#endif
...
.USERRET_USER:
    ebreak
8000086c:	00100073          	ebreak

80000870 <USERRET_MACHINE>:

    .global USERRET_MACHINE
USERRET_MACHINE:
    la s1, uregs
80000870:	007ef497          	auipc	s1,0x7ef
80000874:	79048493          	addi	s1,s1,1936 # 807f0000 <_sbss>
    li s2, TF_SIZE                  // 计数器
80000878:	08000913          	li	s2,128
.LC4:
    lw a0, 0(sp)
8000087c:	00012503          	lw	a0,0(sp)
    sw a0, 0(s1)
80000880:	00a4a023          	sw	a0,0(s1)
    addi s2, s2, -4                 // 滚动计数器
80000884:	ffc90913          	addi	s2,s2,-4
    addi s1, s1, 0x4
80000888:	00448493          	addi	s1,s1,4
    addi sp, sp, 0x4
8000088c:	00410113          	addi	sp,sp,4
    bne s2, zero, .LC4
80000890:	fe0916e3          	bnez	s2,8000087c <USERRET_MACHINE+0xc>

    la s1, uregs
80000894:	007ef497          	auipc	s1,0x7ef
80000898:	76c48493          	addi	s1,s1,1900 # 807f0000 <_sbss>
    LOAD sp, TF_ksp(s1)             // 重新获得当前监控程序栈顶指针
8000089c:	0804a103          	lw	sp,128(s1)

    ori a0, zero, TIMETOKEN         // 发送TIMETOKEN(0x07)信号
800008a0:	00706513          	ori	a0,zero,7
    jal WRITE_SERIAL                // 告诉终端用户程序结束运行
800008a4:	f68ff0ef          	jal	ra,8000000c <WRITE_SERIAL>

    j .DONE
800008a8:	0a40006f          	j	8000094c <.DONE>
...
.USERRET2:
    la ra, uregs              // 定位用户空间寄存器备份地址
800008ac:	007ef097          	auipc	ra,0x7ef
800008b0:	75408093          	addi	ra,ra,1876 # 807f0000 <_sbss>

    //STORE ra, TF_ra(ra)
    STORE sp, TF_sp(ra)
800008b4:	0020a223          	sw	sp,4(ra)
    STORE gp, TF_gp(ra)
800008b8:	0030a423          	sw	gp,8(ra)
    STORE tp, TF_tp(ra)
800008bc:	0040a623          	sw	tp,12(ra)
    STORE t0, TF_t0(ra)
800008c0:	0050a823          	sw	t0,16(ra)
    STORE t1, TF_t1(ra)
800008c4:	0060aa23          	sw	t1,20(ra)
    STORE t2, TF_t2(ra)
800008c8:	0070ac23          	sw	t2,24(ra)
    STORE s0, TF_s0(ra)
800008cc:	0080ae23          	sw	s0,28(ra)
    STORE s1, TF_s1(ra)
800008d0:	0290a023          	sw	s1,32(ra)
    STORE a0, TF_a0(ra)
800008d4:	02a0a223          	sw	a0,36(ra)
    STORE a1, TF_a1(ra)
800008d8:	02b0a423          	sw	a1,40(ra)
    STORE a2, TF_a2(ra)
800008dc:	02c0a623          	sw	a2,44(ra)
    STORE a3, TF_a3(ra)
800008e0:	02d0a823          	sw	a3,48(ra)
    STORE a4, TF_a4(ra)
800008e4:	02e0aa23          	sw	a4,52(ra)
    STORE a5, TF_a5(ra)
800008e8:	02f0ac23          	sw	a5,56(ra)
    STORE a6, TF_a6(ra)
800008ec:	0300ae23          	sw	a6,60(ra)
    STORE a7, TF_a7(ra)
800008f0:	0510a023          	sw	a7,64(ra)
    STORE s2, TF_s2(ra)
800008f4:	0520a223          	sw	s2,68(ra)
    STORE s3, TF_s3(ra)
800008f8:	0530a423          	sw	s3,72(ra)
    STORE s4, TF_s4(ra)
800008fc:	0540a623          	sw	s4,76(ra)
    STORE s5, TF_s5(ra)
80000900:	0550a823          	sw	s5,80(ra)
    STORE s6, TF_s6(ra)
80000904:	0560aa23          	sw	s6,84(ra)
    STORE s7, TF_s7(ra)
80000908:	0570ac23          	sw	s7,88(ra)
    STORE s8, TF_s8(ra)
8000090c:	0580ae23          	sw	s8,92(ra)
    STORE s9, TF_s9(ra)
80000910:	0790a023          	sw	s9,96(ra)
    STORE s10, TF_s10(ra)
80000914:	07a0a223          	sw	s10,100(ra)
    STORE s11, TF_s11(ra)
80000918:	07b0a423          	sw	s11,104(ra)
    STORE t3, TF_t3(ra)
8000091c:	07c0a623          	sw	t3,108(ra)
    STORE t4, TF_t4(ra)
80000920:	07d0a823          	sw	t4,112(ra)
    STORE t5, TF_t5(ra)
80000924:	07e0aa23          	sw	t5,116(ra)
    STORE t6, TF_t6(ra)
80000928:	07f0ac23          	sw	t6,120(ra)

    LOAD sp, TF_ksp(ra)             // 重新获得当前监控程序栈顶指针
8000092c:	0800a103          	lw	sp,128(ra)
    mv a0, ra
80000930:	00008513          	mv	a0,ra
    la ra, .USERRET2
80000934:	00000097          	auipc	ra,0x0
80000938:	f7808093          	addi	ra,ra,-136 # 800008ac <.USERRET2>
    STORE ra, TF_ra(a0)
8000093c:	00152023          	sw	ra,0(a0)

    ori a0, zero, TIMETOKEN         // 发送TIMETOKEN(0x07)信号
80000940:	00706513          	ori	a0,zero,7
    jal WRITE_SERIAL                // 告诉终端用户程序结束运行
80000944:	ec8ff0ef          	jal	ra,8000000c <WRITE_SERIAL>

    j .DONE
80000948:	0040006f          	j	8000094c <.DONE>

8000094c <.DONE>:

.DONE:
    j SHELL                         // 交互循环
8000094c:	d3dff06f          	j	80000688 <SHELL>
	...
